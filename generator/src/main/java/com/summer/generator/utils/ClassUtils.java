...
public abstract class ClassUtils {
     ...
     * 根据类的全路径判断是否可以被实例化
     *
     * @param classRealPath 类全路径
     * @return boolean
     */
    public static boolean isCanInstance(String classRealPath) {
        if (StringUtils.isEmpty(classRealPath) || !isAvailable(classRealPath)) {
            return false;
        }
        Class<?> clazz;
        try {
            clazz = Class.forName(classRealPath);
